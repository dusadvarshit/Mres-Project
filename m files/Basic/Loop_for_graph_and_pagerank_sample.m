...
for i =1:length(all_models)
    ...
    % Sample the flux solution space
    [sampleStruct,mixedFrac] = gpSampler(model1,5000,[],120);
    mf = mixedFrac;
    
    v = zeros(3788,1);
    for j = 1:3788
        v(j) = mean(sampleStruct.points(j,:));
    end
    ...
    save(destination_path, 'model', 'model1', 'M', 'page_rank', 'v', 'mf');
end
