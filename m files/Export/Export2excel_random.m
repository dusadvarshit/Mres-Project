...
for k=1:length(folder_exl)-2
    ...
    backward_index = length(find(find(page_rank_random>page_rank_random(index))<3788));
    for j=1:length(indices)
        if indices(j)<=3788
            ...
            page_rank_1(j) = page_rank_random(indices(j));
            v_rand_1(j) = v_rand(indices(j));
            index_1(j) = indices(j);
        ...
        elseif indices(j)>3788
                ...
                page_rank_2(j-backward_index) = page_rank_random(indices(j));
                v_rand_2(j-backward_index) = v_rand(indices(j)-3788);
    ...
    if length(rxns_1)~=0
        xlswrite(filename,{'rxns','rxnNames','subSystems','page_rank_random','v_rand','index','','reaction_formulas', 'reaction_names'},'Forward','A1');
        ...
        xlswrite(filename,v_rand_1','Forward','E2');
    ...
    if length(rxns_2)~=0
        xlswrite(filename,{'rxns','rxnNames','subSystems','page_rank_random','v_rand','index','','reaction_formulas', 'reaction_names'},'Backward','A1');
        ...
        xlswrite(filename,v_rand_2','Backward','E2');
