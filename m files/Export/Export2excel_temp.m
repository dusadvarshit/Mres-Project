...
folder_exl = dir('C:\Users\dusad\Downloads\M.res Systems and Synthetic Biology\Projects\Diego+Keun\Project work\Cancer models\11.03.2018\Aerobic\HapMap\Mat files');

list_models = cell(length(folder_exl)-2,1);
for i=3:length(folder_exl)
    list_models{i-2} = strcat('C:\Users\dusad\Downloads\M.res Systems and Synthetic Biology\Projects\Diego+Keun\Project work\Cancer models\11.03.2018\Aerobic\HapMap\Mat files','\',folder_exl(i).name);
end
...
for k=1:length(folder_exl)-2
    ...
    disp(list_models{k});
    load(list_models{k});
    filename = 'C:\Users\dusad\Downloads\M.res Systems and Synthetic Biology\Projects\Diego+Keun\Project work\Cancer models\11.03.2018\Aerobic\HapMap\XLSM files';
    ...
    indices = find(page_rank>min(page_rank));
    ...
    backward_index = length(find(find(page_rank>min(page_rank))<3788));
