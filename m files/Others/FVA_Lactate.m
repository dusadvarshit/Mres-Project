...
for i =1:length(all_models)
    ...
    model1.lb(3745) = model2.v(3745);
    model1.ub(3745) = model2.v(3745);

    model1 = changeObjective(model1,model.rxns(941));

    FVA_min = optimizeCbModel(model1,'min');
    FVA_max = optimizeCbModel(model1,'max');
    ...
    save(destination_path, 'model', 'model1', 'FVA_min', 'FVA_max');
