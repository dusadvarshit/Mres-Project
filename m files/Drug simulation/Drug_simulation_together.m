...
cellLine = cell(len,1);
v_NoDrug = zeros(len,1);
v_Methotrexate = zeros(len,1);
v_Raltitrexed = zeros(len,1);
v_Pemetrexed = zeros(len,1);
v_Mercaptopurine = zeros(len,1);
v_Thioguanine = zeros(len,1);
v_Cladribine = zeros(len,1);
v_Fludarabine = zeros(len,1);
v_Clofarabine = zeros(len,1);
v_Florouracil = zeros(len,1);
v_Gemcitabine = zeros(len,1);
v_Capecitabine = zeros(len,1);
v_Hydroxyurea = zeros(len,1);
v_Pentostatin = zeros(len,1);
v_Allopurinol = zeros(len,1);
...
for i =1:length(all_models)
    ...
    model1 = changeRxnBounds(model1,model.rxns(1372),-1,'b'); %L-Glutamine -1
    model1 = changeRxnBounds(model1,model.rxns(968),-5,'b'); % D-Glucose -5
...
filename = 'C:\Users\dusad\Downloads\M.res Systems and Synthetic Biology\Projects\Diego+Keun\Project work\Cancer models\2018.06.13\Drugs Simulation Together\biomass_comparison_Hapmap.xls';
