...
number_nodes = zeros(1000,1);
number_edges = zeros(1000,1);
for i = 1:1000
    load(all_models{i})
    disp(i);
    H =  extract_connected_subgraph(G1);
...
for i=1:1000
    ...
    disp(i);
    clear G1;
    G = digraph(M_random_S,nodenames,'OmitSelfLoops');
    H =  extract_connected_subgraph(G);
...
for i=1:1000
    ...
    h2 = histogram(page_rank_random_S);
    h2.BinWidth = min(page_rank_random_S);
    x = h2.BinEdges(1:length(h2.BinEdges)-1);
    y = h2.BinCounts;
